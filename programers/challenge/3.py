...
def get_most_front(s):
    heap = []
    for i in range(len(s)+1):
        candidate = s[:i] + "110" + s[i:]
...
def solution(s):
    ...
    for e in s:
        ...
        before = e
        while True:
            temp = get_110_deleted(before)
            next = get_most_front(temp)
            if before != next:
                before = next
                continue
            else:
...
print(solution(["0","1110","100111100","0111111010"]))